...
const PageKanoModelCCEI = () => {
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const evaluteData = {
          ...
          business_analysis: response.response.kano_model_business_analysis,
        ...
        while (
          ...
          (!responseEvalute ||
            ...
            responseEvalute?.response?.kano_model_evaluation_education
              .length === 0 ||
            !responseEvalute?.response?.kano_model_evaluation_education.every(
              (persona) =>
                persona.name &&
                Array.isArray(persona.answers) &&
                persona.answers.length > 0 &&
                persona.answers.every(
                  (answer) =>
                    answer.idea_title &&
                    typeof answer.positive_answer === "number" &&
                    typeof answer.negative_answer === "number" &&
                    answer.positive_reason &&
                    answer.negative_reason
                )
            ))
        ) {
      ...
      // Calculate grouped legend data (report data) based on image criteria
      const graphData = responseKanoModel.response.kano_coefficients_result;

      const getQuadrantNameByCoeffs = (csp, csm) => {
        // Classification from the image (lowercase for object keys)
        if (csp >= 0.5 && csm > -0.5) return "attractive";
        if (csp >= 0.5 && csm <= -0.5) return "one_dimensional";
        if (csp < 0.5 && csm <= -0.5) return "must_be";
        if (csp < 0.5 && csm > -0.5) return "indifferent";
        return "indifferent"; // Fallback for items not fitting the specific rules
      };
      ...
      Object.entries(graphData).forEach(([title, data]) => {
        const quadrantName = getQuadrantNameByCoeffs(data.CSP, data.CSM);
  ...
  const handleLegendItemClick = (ideaTitle) => {
    const opinions = kanoModelEvaluation
      .map((persona) => {
        const answer = persona.answers.find(
          (ans) => ans.idea_title === ideaTitle
        );
        if (answer) {
          ...
          return {
            ...
            positive_reason: answer.positive_reason,
            negative_reason: answer.negative_reason,
          };
        }
        return null;
      })
