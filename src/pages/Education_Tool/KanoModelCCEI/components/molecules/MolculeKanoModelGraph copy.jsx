  ...
  const processKanoDataForGraph = (graphData, medianVals) => {
    ...
    const medianX =
      medianVals && medianVals.x !== undefined
        ? medianVals.x
        : (Math.min(...allXValues) + Math.max(...allXValues)) / 2;
    const medianY =
      medianVals && medianVals.y !== undefined
        ? medianVals.y
        : (Math.min(...allYValues) + Math.max(...allYValues)) / 2;

    const maxAbsDistX = Math.max(
      ...allXValues.map((x) => Math.abs(x - medianX))
    );
    const maxAbsDistY = Math.max(
      ...allYValues.map((y) => Math.abs(y - medianY))
    );

    const scale = (value, median, maxDist) => {
      const effectiveMaxDist = maxDist > 0 ? maxDist : 0.1;
      const relativeValue = value - median;
      // Scale relativeValue from [-effectiveMaxDist, effectiveMaxDist] to [5, 95]
      const scaled =
        ((relativeValue - -effectiveMaxDist) /
          (effectiveMaxDist - -effectiveMaxDist)) *
          (95 - 5) +
        5;
      console.log("scaled", scaled);
      return Math.max(5, Math.min(scaled, 95)); // Clamp between 5 and 95
    };
    ...
    const dataPoints = sortedTitles.map((title, index) => {
      ...
      let x = scale(rawX, medianX, maxAbsDistX);
      let y = scale(rawY, medianY, maxAbsDistY);
